...
local function pbar_maximize(time)
  ...
  msg.debug("[MAXIMIZE] received maximize request")
...
local function init()
  ...
  -- NOTE: my old implementation delete this later
  --mp.add_key_binding("o", "show_time", function(event)
  --  if opt.pbar_minimized_height > 0 then
  --    if event.event == "down" then
  --      pbar_update(PBAR_ACTIVE)
  --    elseif event.event == "up" and state.fullscreen then
  --      pbar_update(PBAR_HIDDEN)
  --    elseif event.event == "up" and not state.fullscreen then
  --      pbar_update(PBAR_MINIMIZED)
  --    end
  --  end
  --end, { complex = true })

  --mp.add_key_binding(nil, "show_timeline", function()
  --  if opt.pbar_minimized_height > 0 and (opt.pbar_fullscreen_hide and state.fullscreen) then
  --    pbar_update(PBAR_MINIMIZED)
  --    mp.add_timeout(1.6, function()
  --      pbar_update(PBAR_HIDDEN)
  --    end)
  --  end
  --end)
